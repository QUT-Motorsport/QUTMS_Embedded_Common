...
CC_MotorCommand_t Compose_CC_MotorCommand(uint16_t nodeId, int32_t motorCommand,
		uint8_t motorId) {
	CC_MotorCommand_t p;
...
CC_SetVariable_t Compose_CC_SetVariable(uint16_t nodeId, uint8_t userVariable,
		int32_t userCommand) {
	CC_SetVariable_t p;
...
CC_SetBool_t Compose_CC_SetBool(uint16_t nodeId, uint8_t boolNum,
		uint32_t userBool) {
	CC_SetBool_t p;
...
CC_Roboteq_t Compose_Roboteq_CAN(uint16_t nodeID, uint8_t css, uint8_t n,
